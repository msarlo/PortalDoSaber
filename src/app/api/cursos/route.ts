...
export async function GET(request: Request) {
  ...
  // Filtrar por role (Saude ou Comum)
  if (role) {
    if (role !== 'Saude' && role !== 'Comum') {
      return NextResponse.json({ error: 'Role inválida. Use "Saude" ou "Comum"' }, { status: 400 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    // Validar role
    if (body.role !== 'Saude' && body.role !== 'Comum') {
      return NextResponse.json(
        { error: 'Role inválida. Use "Saude" ou "Comum"' }, 
        { status: 400 }
      );
    }
...
export async function PUT(request: Request) {
  try {
    ...
    if (body.role && body.role !== 'Saude' && body.role !== 'Comum') {
      return NextResponse.json(
        { error: 'Role inválida. Use "Saude" ou "Comum"' }, 
        { status: 400 }
      );
