...
export const cadastroSchema = z
  .object({
    ...
    email: z.string().email("Forneça um e-mail válido").trim(),
...
export const loginSchema = z.object({
  email: z.string().email("Forneça um e-mail válido").trim(),
  senha: z.string().min(1, "Senha é obrigatória"), // Simplificado para login
});
...
// Tipos para backend
export type CriarUsuarioBackendInput = z.infer<
  typeof criarUsuarioBackendSchema
>;
...
export function converterCadastroParaBackend(
  dadosCadastro: CadastroFormData
): CriarUsuarioBackendInput {
